...
class DockerUniverseBackend(UniverseBackend):
    """Run a Universe inside a Docker container using the Docker CLI.
    ...
    - Docker publishes that container port to a host endpoint, discovered with ``docker port``.
    """
    ...
    def launch(self, spec: UniverseRuntimeSpec, config: Dict[str, Any] | None = None) -> UniverseHandle:
        ...
        docker_bin = str(config.get("docker_bin") or os.environ.get("WOLF_DOCKER_BIN") or "docker")
        image = str(
            config.get("image")
            or os.environ.get("WOLF_UNIVERSE_DOCKER_IMAGE")
        ...
        if config.get("user") is not None:
            cmd.extend(["--user", str(config["user"])])
        if config.get("userns") is not None:
            cmd.extend(["--userns", str(config["userns"])])
        ...
        if _bool_config(config, "read_only", False):
        ...
        process_handle = {"container_id": container_id, "container_name": container_name, "docker_bin": docker_bin}
        ...
        while elapsed < max_wait:
            endpoint = self._discover_published_endpoint(handle, container_port=container_port, publish_host=publish_host, docker_bin=docker_bin)
    ...
    def _run_docker(self, handle: UniverseHandle, args: list[str]) -> subprocess.CompletedProcess:
        docker_bin = str((handle.process or {}).get("docker_bin") or handle.metadata.get("docker_bin") or "docker") if isinstance(handle.process, dict) else "docker"
        return subprocess.run([docker_bin, *args], capture_output=True, text=True, check=False)
    ...
    def _container_running(self, handle: UniverseHandle) -> bool:
        result = self._run_docker(handle, ["inspect", "-f", "{{.State.Running}}", self._container_ref(handle)])
        if result.returncode != 0:
            return False
        return (result.stdout or "").strip().lower() == "true"

    def _discover_published_endpoint(self, handle: UniverseHandle, *, container_port: int, publish_host: str, docker_bin: str) -> UniverseEndpoint | None:
        ref = self._container_ref(handle)
        result = subprocess.run([docker_bin, "port", ref, f"{int(container_port)}/tcp"], capture_output=True, text=True, check=False)
        if result.returncode != 0:
            return None
    ...
    def status(self, handle: UniverseHandle) -> UniverseStatus:
        ...
        docker_bin = str((handle.process or {}).get("docker_bin") or "docker") if isinstance(handle.process, dict) else "docker"
        endpoint = self._discover_published_endpoint(handle, container_port=container_port, publish_host=publish_host, docker_bin=docker_bin) or handle.endpoint
    ...
    def logs(self, handle: UniverseHandle, tail: int = 200) -> Dict[str, str]:
        result = self._run_docker(handle, ["logs", "--tail", str(max(1, int(tail))), self._container_ref(handle)])
        if result.returncode != 0:
            return {"stdout": result.stdout or "", "stderr": result.stderr or f"docker logs failed with returncode {result.returncode}"}
    ...
    def terminate(self, handle: UniverseHandle, force: bool = False, timeout: float = 10.0) -> UniverseStatus:
        ref = self._container_ref(handle)
        if force:
            result = self._run_docker(handle, ["rm", "-f", ref])
        else:
            result = self._run_docker(handle, ["stop", "--time", str(max(0, int(timeout))), ref])
